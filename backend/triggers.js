...
module.exports = {
    productSaleUpdate: async function(pool, storename) {
        try {
    ...
    afterPurchaseUpdate: async function(pool, storename) {
        try {
            // await pool.query(`USE \`${storename}\`;`);
            // Drop the trigger if it already exists
            ...
            await pool.query(`
                ...
                FOR EACH ROW 
                BEGIN 
                    -- Check if the orderStatus is 1 and the product is not new
